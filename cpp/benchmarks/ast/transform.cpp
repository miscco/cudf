...
static void BM_ast_transform(nvbench::state& state)
{
  ...
  std::for_each(
    cuda::make_counting_iterator(0), cuda::make_counting_iterator(num_columns), [&](int column_id) {
  ...
  if (reuse_columns) {
    ...
    std::for_each(
      cuda::make_counting_iterator(2), cuda::make_counting_iterator(num_columns), [&](int col_id) {
...
static void BM_string_compare_ast_transform(nvbench::state& state)
{
  ...
  std::for_each(
    cuda::make_counting_iterator(0), cuda::make_counting_iterator(num_columns), [&](size_t) {
  ...
  std::for_each(
    cuda::make_counting_iterator(1), cuda::make_counting_iterator(tree_levels), [&](size_t idx) {
