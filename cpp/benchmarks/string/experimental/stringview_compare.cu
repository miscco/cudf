  ...
  cudf::strings_column_view const& input, rmm::cuda_stream_view stream)
{
  ...
  auto const num_longer_strings = thrust::count_if(
    ...
    cuda::make_counting_iterator<cudf::size_type>(input.size()),
...
static void BM_sv_hash(nvbench::state& state)
{
  ...
  auto begin  = cuda::make_counting_iterator<cudf::size_type>(0);
  auto end    = cuda::make_counting_iterator<cudf::size_type>(num_rows);
...
static void BM_sv_sort(nvbench::state& state)
{
  ...
  auto in_keys   = cuda::make_counting_iterator<cudf::size_type>(0);
