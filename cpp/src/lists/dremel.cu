                         ...
                         rmm::cuda_stream_view stream)
{
  ...
  size_type curr_rep_values_size = 0;
  {
    ...
    auto input_child_def_it =
      thrust::make_transform_iterator(cuda::make_counting_iterator(column_offsets[level + 1]),
    ...
    auto ends = thrust::merge_by_key(rmm::exec_policy_nosync(stream),
                                     ...
                                     cuda::make_counting_iterator(column_offsets[level + 1]),
                                     cuda::make_counting_iterator(column_ends[level + 1]),
                                     input_parent_zip_it,
                                     input_child_zip_it,
