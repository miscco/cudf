  ...
  rmm::device_async_resource_ref mr)
{
  ...
  return cudf::detail::valid_if(cuda::make_counting_iterator<size_type>(0),
                                cuda::make_counting_iterator<size_type>(num_child_rows),
                                is_valid_predicate,
                                stream,
                                mr);
}
 ...
 public:
    ...
    requires(cudf::is_fixed_width<T>())
  {
    ...
    thrust::transform(
      ...
      cuda::make_counting_iterator(child_column->size()),
    ...
    requires(std::is_same_v<T, string_view>)
  {
    ...
    thrust::transform(
      ...
      cuda::make_counting_iterator<size_type>(string_views.size()),
      string_views.begin(),
    ...
    requires(std::is_same_v<T, list_view>)
  {
    ...
    thrust::transform(
      rmm::exec_policy_nosync(stream),
      cuda::make_counting_iterator<size_type>(0),
      cuda::make_counting_iterator<size_type>(child_list_views.size()),
    ...
    requires(std::is_same_v<T, struct_view>)
  {
    ...
    auto const iter_source_member_as_list = thrust::make_transform_iterator(
      cuda::make_counting_iterator<cudf::size_type>(0), [&](auto child_idx) {
