  ...
  for (auto& column : columns) {
    ...
    auto RLE_column_size = [&](TypeKind type_kind) {
      return std::accumulate(
        ...
        cuda::make_counting_iterator(segmentation.num_rowgroups()),
        0ul,
                             ...
                             rmm::cuda_stream_view stream)
{
  thrust::for_each(rmm::exec_policy_nosync(stream),
                   cuda::make_counting_iterator(0ul),
                   cuda::make_counting_iterator(stat_desc.size()),
                              ...
                              rmm::cuda_stream_view stream)
{
  ...
  thrust::for_each_n(
    rmm::exec_policy_nosync(stream),
    cuda::make_counting_iterator(0u),
    col.size(),
  ...
  for (auto col_idx : orc_table.string_column_indices) {
    ...
    for (auto const& stripe : segmentation.stripes) {
      ...
      auto const direct_char_count = std::accumulate(
        cuda::make_counting_iterator(stripe.first),
        cuda::make_counting_iterator(stripe.first + stripe.size),
        0,
  ...
  if (sort_dictionaries) {
    ...
    for (auto& sd : dictionaries_flat) {
      ...
      thrust::scatter(rmm::exec_policy_nosync(current_stream),
                      ...
                      cuda::counting_iterator<uint32_t>(sd.data_order.size()),
                      order_copy.begin(),
                      sd.data_order.begin());
