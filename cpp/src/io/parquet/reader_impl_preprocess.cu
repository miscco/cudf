...
void reader_impl::allocate_nesting_info()
{
  ...
  auto iter = cuda::make_counting_iterator(size_type{0});
  std::transform(iter, iter + num_columns, per_page_nesting_info_size.begin(), [&](size_type i) {
    ...
    std::for_each(cuda::make_counting_iterator(static_cast<size_t>(1)),
                  cuda::make_counting_iterator(_num_sources),
  ...
  auto counting_iter = cuda::make_counting_iterator(size_t{0});
  ...
  for (size_t idx = 0; idx < _input_columns.size(); idx++) {
    ...
    std::for_each(
      cuda::make_counting_iterator(static_cast<size_t>(0)),
      cuda::make_counting_iterator(_num_sources),
      [&](auto const src_file_idx) {
...
void reader_impl::allocate_columns(read_mode mode, size_t skip_rows, size_t num_rows)
{
  ...
  if (has_lists) {
    ...
    while (key_start < num_keys) {
      ...
      thrust::transform(
        rmm::exec_policy_nosync(_stream),
        cuda::make_counting_iterator<size_t>(key_start),
        cuda::make_counting_iterator<size_t>(key_start + num_keys_this_iter),
        size_input.begin(),
      ...
      auto const reduction_keys =
        thrust::make_transform_iterator(cuda::make_counting_iterator<std::size_t>(key_start),
