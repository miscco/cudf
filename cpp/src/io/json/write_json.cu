                                          ...
                                          rmm::device_async_resource_ref mr)
{
  ...
  if (strings_columns.num_columns() > 0) {
    ...
    thrust::for_each(rmm::exec_policy_nosync(stream),
                     ...
                     cuda::make_counting_iterator<size_type>(total_rows),
                     scatter_fn);
  } else {
    thrust::for_each(
      rmm::exec_policy_nosync(stream),
      cuda::make_counting_iterator<size_type>(0),
      cuda::make_counting_iterator<size_type>(num_rows),
                                             ...
                                             rmm::device_async_resource_ref mr)
{
  ...
  */
  ...
  thrust::for_each(rmm::exec_policy_nosync(stream),
                   cuda::make_counting_iterator<size_type>(0),
                   cuda::make_counting_iterator<size_type>(num_lists),
                   [col = *col_device_view,
                    list_prefix,
                    list_suffix,
  ...
  std::vector<column_name_info> user_column_names = [&]() {
    ...
    if (metadata.has_value() and not metadata->schema_info.empty()) {
      ...
      std::transform(cuda::make_counting_iterator(0),
                     cuda::make_counting_iterator(table.num_columns()),
