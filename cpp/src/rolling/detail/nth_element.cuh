...
struct gather_index_calculator {
  ...
  size_type __device__ operator()(size_type i) const
  {
    ...
    return n >= 0 ? index_of_nth_non_null(cuda::make_counting_iterator(window_start), window_size)
                  : index_of_nth_non_null(
                      cuda::std::make_reverse_iterator(cuda::make_counting_iterator(window_end)),
                      window_size);
  }
};
