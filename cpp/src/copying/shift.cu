                                                          ...
                                                          rmm::device_async_resource_ref mr)
{
  ...
  return detail::valid_if(cuda::make_counting_iterator<size_type>(0),
                          cuda::make_counting_iterator<size_type>(size),
                          func_validity,
                          stream,
                          mr);
}
...
struct shift_functor {
    ...
    requires(cudf::is_fixed_width<T>())
  {
    ...
    auto const size  = input.size();
    auto index_begin = cuda::make_counting_iterator<size_type>(0);
    auto index_end   = cuda::make_counting_iterator<size_type>(size);
    ...
    if (not scalar_is_valid) {
      ...
      if (offset > 0) {
        index_begin = cuda::make_counting_iterator<size_type>(offset);
        data        = data + offset;
      } else if (offset < 0) {
        index_end = cuda::make_counting_iterator<size_type>(size + offset);
      }
    }
