                                           ...
                                           rmm::device_async_resource_ref mr)
  {
    ...
    auto const create_mask_fn = [&] {
      ...
      return cudf::detail::valid_if(cuda::make_counting_iterator<size_type>(0),
                                    cuda::make_counting_iterator<size_type>(output_size),
                                    validity_fn,
                                    stream,
                                    mr);
    };
...
struct interleave_columns_impl<T, std::enable_if_t<std::is_same_v<T, cudf::string_view>>> {
                                           ...
                                           rmm::device_async_resource_ref mr)
  {
    ...
    thrust::transform(rmm::exec_policy_nosync(stream),
                      ...
                      cuda::make_counting_iterator<size_type>(num_strings),
                      indices.begin(),
...
struct interleave_columns_impl<T, std::enable_if_t<cudf::is_fixed_width<T>()>> {
                                           ...
                                           rmm::device_async_resource_ref mr)
  {
    ...
    auto index_end     = cuda::make_counting_iterator<size_type>(output_size);
