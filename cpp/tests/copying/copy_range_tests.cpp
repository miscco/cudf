...
TYPED_TEST(CopyRangeTypedTestFixture, CopyNoNulls)
{
  ...
  cudf::test::fixed_width_column_wrapper<T, int32_t> target(cuda::make_counting_iterator(0),
                                                            cuda::make_counting_iterator(0) + size);
...
TEST_F(CopyRangeTestFixture, CopyDictionary)
{
  ...
  auto source_validity = thrust::make_transform_iterator(
    cuda::make_counting_iterator<cudf::size_type>(0), [](auto i) { return i != 3; });
  auto target_validity = thrust::make_transform_iterator(
    cuda::make_counting_iterator<cudf::size_type>(0), [](auto i) { return i != 3 && i != 9; });
  auto expected_validity = thrust::make_transform_iterator(
    cuda::make_counting_iterator<cudf::size_type>(0), [](auto i) { return i != 5 && i != 9; });
  {
...
TEST_F(CopyRangeErrorTestFixture, InvalidInplaceCall)
{
  ...
  auto target = cudf::test::fixed_width_column_wrapper<int32_t>(
    cuda::make_counting_iterator(0), cuda::make_counting_iterator(0) + size);
...
TYPED_TEST(FixedPointTypesCopyRange, FixedPointLarge)
{
  ...
  auto s = cuda::make_counting_iterator(-1000);
