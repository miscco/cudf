...
TYPED_TEST(RepeatTypedTestFixture, RepeatScalarCount)
{
  ...
  auto input = cudf::test::fixed_width_column_wrapper<T, int32_t>(
    cuda::make_counting_iterator(0), cuda::make_counting_iterator(0) + num_values);
...
TYPED_TEST(RepeatTypedTestFixture, ZeroSizeInput)
{
  ...
  cudf::test::fixed_width_column_wrapper<T, int32_t> input(cuda::make_counting_iterator(0),
                                                           cuda::make_counting_iterator(0));

  auto count = cudf::test::fixed_width_column_wrapper<cudf::size_type>(
    cuda::make_counting_iterator(0), cuda::make_counting_iterator(0));

  cudf::test::fixed_width_column_wrapper<T, int32_t> expected(cuda::make_counting_iterator(0),
                                                              cuda::make_counting_iterator(0));
...
TEST_F(RepeatErrorTestFixture, LengthMismatch)
{
  auto input = cudf::test::fixed_width_column_wrapper<int32_t>(
    cuda::make_counting_iterator(0), cuda::make_counting_iterator(0) + 100);

  auto count = cudf::test::fixed_width_column_wrapper<cudf::size_type>(
    cuda::make_counting_iterator(0), cuda::make_counting_iterator(0) + 200);
...
TEST_F(RepeatErrorTestFixture, CountHasNulls)
{
  ...
  auto count =
    cudf::test::fixed_width_column_wrapper<cudf::size_type>(cuda::make_counting_iterator(0),
                                                            cuda::make_counting_iterator(0) + 100,
