...
TEST_F(RollingErrorTest, SumTimestampNotSupported)
{
  ...
  cudf::test::fixed_width_column_wrapper<cudf::timestamp_D, cudf::timestamp_D::rep> input_D(
    cuda::make_counting_iterator(0), cuda::make_counting_iterator(size));
...
TYPED_TEST(RollingTest, RandomDynamicAllValid)
{
  ...
  auto it = cuda::make_counting_iterator<cudf::size_type>(0);
...
TEST_F(RollingTestUdf, StaticWindow)
{
  ...
  cudf::test::fixed_width_column_wrapper<int32_t> input(cuda::make_counting_iterator(0),
  ...
  auto start = cudf::detail::make_counting_transform_iterator(0, [size](cudf::size_type row) {
    return std::accumulate(cuda::make_counting_iterator(std::max(0, row - 2 + 1)),
                           cuda::make_counting_iterator(std::min(size, row + 2 + 1)),
                           0);
  });
...
TEST_F(RollingTestUdf, DynamicWindow)
{
  ...
  auto start =
    cudf::detail::make_counting_transform_iterator(0, [size] __device__(cudf::size_type row) {
      return std::accumulate(cuda::make_counting_iterator(std::max(0, row - (row % 2 + 2) + 1)),
                             cuda::make_counting_iterator(std::min(size, row + (row % 2) + 1)),
                             0);
    });
