                                                ...
                                                rmm::cuda_stream_pool& stream_pool)
{
  ...
  std::for_each(
    cuda::make_counting_iterator(0), cuda::make_counting_iterator(thread_count), [&](auto tid) {
                                             ...
                                             rmm::cuda_stream_view stream)
{
  ...
  std::for_each(
    cuda::make_counting_iterator(1), cuda::make_counting_iterator(input_multiplier), [&](auto i) {
...
int32_t main(int argc, char const** argv)
{
  ...
  // Read the same parquet files specified times with multiple threads and discard the read tables
  {
    ...
    std::for_each(cuda::make_counting_iterator(0),
                  cuda::make_counting_iterator(num_reads),
